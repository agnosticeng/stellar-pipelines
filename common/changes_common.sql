...
create table ledgers_{{.RANGE_START}}_{{.RANGE_END}} engine=StripeLog as (
    ...
    select
        ...
        _upgrade_processing_changes_raw
    from ledgers
)
...
{{define "changes"}}
    ...
    upgrade_processing_changes as (
        select
            ...
            'upgrade_processing_changes' as source,
            _change_raw
        from ledgers_{{.RANGE_START}}_{{.RANGE_END}}
    ...
    fee_processing_changes as (
        select
            ...
            null as operation_result_code,
            null as operation_inner_result_code,
            'fee_processing_changes' as source,
            _change_raw
        from txs_{{.RANGE_START}}_{{.RANGE_END}}
    ...
    post_tx_apply_fee_processing_changes as (
        select
            ...
            null as operation_result_code,
            null as operation_inner_result_code,
            'post_tx_apply_fee_processing_changes' as source,
            _change_raw
        from txs_{{.RANGE_START}}_{{.RANGE_END}}
...
{{define "changes_common_drop_ledgers"}}

drop table ledgers_{{.RANGE_START}}_{{.RANGE_END}}

{{end}}

{{define "changes_common_drop_txs"}}
